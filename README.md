...
**三种模式**：
- **周扫**（`/paper-radar`）：四条固定线全扫——硬件 / 模型 / 软件系统 / 工程方法，每线必报，没货也要写「本线无值得推的」；另出「工程实践」节收非论文
...
- **信源分级与隔离**：论文清单（四条线）只收论文；工程博客 / 技术报告 / 上游变更进独立的「工程实践」节，X 快讯单列。分级标注、独立配额，绝不混排——博客好读好写，混排会稳定挤掉论文
- 每条必带可点击链接，强制标注来源与溯源级别；论文首推 2–3 篇必须全文验证，非论文详条目必须读原文并过来源核验
- 首推「本周只读一篇」恒为论文；论文 ≤8 篇（丰收周可声明扩容到 12）+ 实践 ≤4 + 快讯 ≤5

**两个自持账本**（这是本技能最值钱的部分）：
- `state/seen-papers.md`：去重账本，已推不重发，防改名重投
- `state/judgments.md`：跨来源判断账本，持续生长，可被新证据标【存疑】。非论文只能作辅助证据，且不能单独把一条判断标【存疑】

**图表**：每期自带三样视觉件——开头的本期体量图（各线条目数、配额线、全文与仅摘要占比）、判断账本动态里的 mermaid 关系图、末尾的跨期趋势图。SVG 由 `scripts/render_charts.py` 生成，只用 Python 标准库，跟随阅读器的浅色深色主题。首推论文的关键图在**许可证是 CC 时**才嵌入并标注出处，其余只给链接。

**表述**：报告定稿前整篇过 `humanizer-zh` 去 AI 痕迹，目标是好读好懂。条目头行格式、【】标注、「谁该读」这些结构件保留不动。
