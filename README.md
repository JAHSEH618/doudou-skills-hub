...
AI 工程化论文雷达，产出简报式报告。为个人技术认知服务，**不做**「你们能不能落地」的业务过滤——每篇标注「谁该读」，把适用性判断留给读者。

**三种模式**：
- **周扫**（`/paper-radar`）：四条固定线全扫——硬件 / 模型 / 软件系统 / 工程方法，每线必报，没货也要写「本线无值得推的」；非论文另出「工程实践」「资讯」「快讯」三节
...
**核心约束**：
- 主动调用 exa MCP 多角度搜论文；`scripts/fetch_sources.py` 拉 AIHot（aihot.news 匿名 API）+ HN Algolia + 固定 X 账号清单出非论文候选池。任一信源不可用必须在报告开头声明
- **信源分级与隔离**：四条线只收论文；工程博客 / 技术报告 / 上游变更进「工程实践」（详条目读原文核验，简条目仅摘要）；有第一方公告页的事件进「资讯」；只有 X 帖的进「快讯」。分级标注、独立配额，绝不混排
- 每条必带可点击链接，强制标注来源与溯源级别；论文首推 2–3 篇必须全文验证
- 首推「本周只读一篇」恒为论文；论文 ≤8 篇（丰收周可声明扩容到 12）+ 实践详 ≤6 简 ≤8 + 资讯 ≤8 + 快讯 ≤8

**两个自持账本**（这是本技能最值钱的部分）：
- `state/seen-papers.md`：去重账本，已推不重发，防改名重投
- `state/judgments.md`：跨来源判断账本，持续生长，可被新证据标【存疑】。非论文只能作辅助证据，且不能单独把一条判断标【存疑】；资讯 / 快讯 / 解说一律不作证据

**图表**：每期三张 SVG 全在「判断账本动态」节——本期体量图（各区条目数与配额线）、判断账本关系图（本期来源 → 动了哪几条 J，替代了手写 mermaid）、跨期趋势图。一份 `stats.json` 是唯一输入，`scripts/render_charts.py` 只用 Python 标准库生成，跟随阅读器的浅色深色主题。首推论文的关键图在**许可证是 CC 时**才嵌入并标注出处。
...
**画像要点**：Java 收口。**无 Java 后端经验一票否决**；通过者分两档——P1 纯 Java 后端为首选，P2 为 Java 打底多年后转型 AI 应用开发，排序恒在 P1 后。全栈定位、短年限双栈、无长 Java 打底的 AI 应用主体均判淘汰。AI coding 工具实证和 SDD/TDD 是必过门槛。

**输出形态**：默认只打印一个代码块的通过名单（HR 可直接群发），档位 / 分数 / 判定原句照常产出但写进 `checked.md`，追问才展开。
